...
add_text_watermark() {
    ...
    local output_file="${video_file%.*}_watermarked.mp4"
    ...
    if $FFMPEG_CMD -i "$video_file" \
        ...
        -codec:a copy \
        "$output_file"; then
        show_success "Text watermark added: $output_file"
    else
...
add_image_watermark() {
    ...
    read -p "Select watermark image number: " image_choice
    ...
    if $FFMPEG_CMD -i "$video_file" -i "$watermark_image" \
        -filter_complex "overlay=$position" \
        -codec:a copy \
        "$output_file"; then
        show_success "Image watermark added: $output_file"
    else
...
add_custom_watermark() {
    ...
    local output_file="${video_file%.*}_custom_wm.mp4"
    ...
    if $FFMPEG_CMD -i "$video_file" \
        ...
        -codec:a copy \
        "$output_file"; then
        show_success "Custom watermark added: $output_file"
    else
...
preview_watermark() {
    ...
    local output_file="${video_file%.*}_preview.mp4"
    
    show_info "Creating 10-second preview with watermark..."
    
    if $FFMPEG_CMD -t 10 -i "$video_file" \
        ...
        "$output_file"; then
        show_success "Preview created: $output_file"
