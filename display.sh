...
show_detailed_video_info() {
    ...
    echo -e "Path:     $(realpath "$video_file")"
    
    if command -v file &> /dev/null; then
        echo -e "Type:     $(file -b "$video_file")"
    fi
...
show_storage_info() {
    ...
    if [[ -d "$BASE_DIR" ]]; then
        ...
        echo -e "Temp: $TEMP_DIR ($(du -sh "$TEMP_DIR" 2>/dev/null | cut -f1 || echo 'Empty'))"
    else
        echo -e "Not setup yet"
    fi
    
    echo
    echo -e "${CYAN}System Storage:${NC}"
    ...
    if [[ $video_count -gt 0 ]]; then
        ...
        for file in *.{mp4,avi,mkv,mov,webm} 2>/dev/null; do
            if [[ -f "$file" ]]; then
                ...
                echo -e "• $file - $(format_file_size $size) - ${duration:-0}s"
            fi
        done
    fi
}
...
show_system_info() {
    ...
    echo -e "System: $(uname -o)"
    echo -e "Architecture: $(uname -m)"
    echo -e "Kernel: $(uname -r)"
    
    echo
    echo -e "${CYAN}FFmpeg Information:${NC}"
