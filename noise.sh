...
remove_video_noise() {
    ...
    local output_file="${video_file%.*}_denoised.mp4"
    ...
    if [[ $? -eq 0 ]]; then
        show_success "Noise removal completed: $output_file"
...
remove_audio_noise() {
    ...
    local output_file="${audio_file%.*}_cleaned.mp3"
    
    show_info "Removing audio noise..."
    
    if $FFMPEG_CMD -i "$audio_file" \
        -af "highpass=300,lowpass=3000,afftdn=nf=-20" \
        "$output_file"; then
        show_success "Audio noise removal completed: $output_file"
    else
...
enhance_audio() {
    ...
    local output_file="${audio_file%.*}_enhanced.mp3"
    ...
    if $FFMPEG_CMD -i "$audio_file" \
        ...
        "$output_file"; then
        show_success "Audio enhancement completed: $output_file"
    else
