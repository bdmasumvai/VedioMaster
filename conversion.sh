...
convert_to_audio() {
    ...
    local output_file="${video_file%.*}.mp3"
    ...
    if $FFMPEG_CMD -i "$video_file" \
        -q:a 0 -map a \
        "$output_file"; then
        show_success "Audio extraction completed: $output_file"
    else
...
convert_to_gif() {
    ...
    local output_file="${video_file%.*}.gif"
    
    show_info "Creating GIF..."
    
    if $FFMPEG_CMD -t "$duration" -i "$video_file" \
        -vf "fps=10,scale=$width:-1:flags=lanczos" \
        "$output_file"; then
        show_success "GIF creation completed: $output_file"
    else
...
convert_avi_to_mp4() {
    ...
    read -p "Select AVI file number: " selection
    ...
    local output_file="${video_file%.*}.mp4"
    ...
    if $FFMPEG_CMD -i "$video_file" \
        -c:v libx264 -c:a aac \
        "$output_file"; then
        show_success "Conversion completed: $output_file"
    else
...
convert_mkv_to_mp4() {
    ...
    read -p "Select MKV file number: " selection
...
convert_webm_to_mp4() {
    ...
    read -p "Select WEBM file number: " selection
...
batch_convert() {
    ...
    for video_file in *.mp4 *.avi *.mkv *.webm *.mov; do
        if [[ -f "$video_file" ]]; then
            local output_file="${video_file%.*}.$target_ext"
            ...
            if [[ $? -eq 0 ]]; then
                show_success "Converted: $output_file"
                ((converted++))
            else
